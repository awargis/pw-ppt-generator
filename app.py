...
def process_pipeline():
    ...
    pages = render_pdf(pdf_file.read(), dpi=300)
    
    questions, headers = [], []
    total, completed = len(pages) * 2, 0
    progress, status = st.progress(0), st.empty()

    for p_idx, page in enumerate(pages):
        for c_idx, (col_img, off_x, off_y) in enumerate(split_page_columns(page)):
            completed += 1
                ...
                try:
                    ...
                    if item["type"] == "question":
                        questions.append(DetectedItem(
                            kind="question", page_index=p_idx, column_index=c_idx, box=box,
                            question_number=int(item["question_number"]), confidence=item.get("confidence", 0.9)
                        ))
                    elif item["type"] == "section_header":
                        headers.append(DetectedItem(
                            kind="section_header", page_index=p_idx, column_index=c_idx, box=box,
                            subject=normalize_subject(item["subject"], settings["subjects"])
                        ))
    ...
    for q in questions:
        ...
        try:
            img = crop_question(pages[q.page_index], q, settings["pad_x"], settings["pad_y"])
            sub = current_subject or "Unclassified"
            grouped.setdefault(sub, []).append({
                "number": q.question_number, "image": img, "page_index": q.page_index
            })
...
if st.button("🚀 Process & Generate", type="primary"):
    ...
    try:
        subject_questions = process_pipeline()
        ...
        render_counts({s: len(i) for s, i in subject_questions.items()})
        render_question_preview(subject_questions)
        
        st.download_button(
            label="📦 Download Formatted PPTs",
            data=zip_buffer,
            file_name=f"{settings['exam_type']}_Presentations.zip",
